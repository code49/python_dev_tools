...
class finalCommands():
    ...
    def dashedFormattedLine(self, string_list):
        ...
        if type(string_list) != "list" or not len(string_list) >= 1:
            self.errorCreator.printError("TypeError", "finalCommands:dashedFormmatedLine()", "expected a list of at least length 1, either didn't get a list or got an empty one",
                                         "something has gone wrong, please inform the creator of the program", "", exit=True)
