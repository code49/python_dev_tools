...
class queryManager():
    ...
    def __indexer__(self, find=None, index=None)->dict:
        ...
        if find != None:
            ...
            if index == None:
                self.errorCreator.printError("ParameterError", "queryManager:__indexer__()", "expected find or index, but got neither.", "something went wrong with response queries.", "", True)

            #check that find is an integer; throw error if not
            if type(find) != int:
                find_type = type(find)
                self.errorCreator.printError("TypeError", "queryManager:__indexer__()", "expected find to be of type int, instead received {find_type}.", "something went wrong with reqponse queries.", "", True)

            #return the dictionary of the requested history_list entry
            try:
                return self.history_list[find]
            except Exception as e:
                self.errorCreator.printError("Exception", "queryManager:__indexer__()", "unknown", e, True)
        ...
        #requested mode is 'index'
            ...
            if type(index) != dict:
                ...
                self.errorCreator.printError("TypeError", "queryManager:__indexer__()", "expected index to be of type dict, instead received {index_type}.", "something went wrong with reqponse queries.", "", True)
    ...
    def baseRequest(self, query)->str:
        ...
        if user_response == None or user_response == "":
            ...
            self.errorCreator.printWarning("queryManager:baseRequest()", "got null user input, please write something and try again")
    ...
    def numberRequest(self, query, lower_bound=None, upper_bound=None, ensure_whole=False, record=False)->userResponse:
        ...
        try:
            ...
            self.errorCreator.printWarning("queryManager:numberRequest()", "input requires a number, but did not receive one")

            #let the user try again
            user_response = self.numberRequest(query, lower_bound=lower_bound, upper_bound=upper_bound, ensure_whole=ensure_whole, record=False)
            
            #create a userRequest or simple return depending on what is requested
            if record:
                response = userResponse(user_response, user_response, query, self.dev_mode)
        ...
        if ensure_whole and user_response % 1 != 0:
            ...
            self.errorCreator.printWarning("queryManager:numberRequest()", "input requires a whole number, but did not receive one")
        ...
        if user_response <= lower_bound:
            ...
            self.errorCreator.printWarning("queryManager:numberRequest()", "need a numeric input greater than or equal to {str_lower_bound}, but instead got {str_user_response}, which is too low")
            ...
            self.errorCreator.printWarning("queryManager:numberRequest", "need a numeric input less than or equal to {str_upper_bound}, but instead got {str_user_response}, which is too high")
    ...
    def stringRequest(self, query, cannot_contain=[], censor=False, processing=[], record=False)->userResponse:
        ...
        if len(matches) != 0:
            if censor:
                ...
                self.errorCreator.printWarning("queryManager:stringRequest()", "the string you inputted contains 'banned' content, please try again")
                user_response = self.stringRequest(query, cannot_contain=cannot_contain, censor=censor, processing=processing, record=False)
    ...
    def optionRequest(self, query, options_list, clear=True, add_previous=False)->userResponse:
        ...
        if len(self.history_list) < 1 and add_previous:
            self.errorCreator.printError("ParameterError", "queryManager:optionRequest()", "tried to add 'back button' functionality without a previous query", "something went wrong with getting your input.", "", True)
        ...
        user_response = self.numberRequest(query, 0, len(options_list) + 1, True, False)

        #input is fine, create a userResponse instance accordingly
        response = userResponse(options_list[user_response], user_response, self.dev_mode)
