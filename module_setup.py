...
class moduleSetup(self):

    def __init__(self, dev_mode, module_list):
    ...
    if !dev_mode:
        ...
        if not data["module_setup_status"]:
            ...
            print(
                "Module Installation: This project has not been set up yet. Installing modules now.")
            ...
            def getPip():
                ...
                if not isfile(pipPath):
                    ...
                    if not isfile(pipPath):
                        raise NameError(
                            "MODULE INSTALL ERROR: failed to find/install pip, try manually installing pip and trying again, or if that doesn't work manually set pipPath to the pip.exe filepath")
            ...
            def installIfNeeded(module_pip_name):
                ...
                try:  # ideally, pip should be able to find the module needed based on the given name, but in some cases the name might be wrong, and that should raise an error
                    ...
                    raise KeyError(
                        f"MODULE INSTALL ERROR: failed to install module {module_pip_name}. check that the module name is correct and then try again.")
            ...
            print(
                "Module Installation: The project is now set up, you should not need to do this again.")
            time.sleep(2)
